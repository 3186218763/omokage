...
from .sentence_pipeline import SentencePipeline
...
class Orchestrator:
    ...
    async def chat(
        ...
        committed = False
        try:
            messages = await ContextBuilder(self._llm).build(conversation)
            ...
            try:
                async for token in self._llm.stream_chat(messages):
                    for utterance in pipeline.feed(token):
                        await tts_queue.put(utterance.text)
                        yield utterance.text
                for utterance in pipeline.close():
                    await tts_queue.put(utterance.text)
                    yield utterance.text
                normalized_response = normalize_speech_text(
                    strip_style_for_history(pipeline.speech_text)
                )
